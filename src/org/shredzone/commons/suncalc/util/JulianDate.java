...
public class JulianDate {
     ...
     *
     * @param cal {@link Calendar} to use. Do not modify this object after
     * invocation.
     */
    public JulianDate(Calendar cal) {
        this.cal = cal;
        mjd = cal.getTime().getTime() / 86400000.0 + 40587.0;
    }

    /**
     * Returns a {@link JulianDate} of the current date and the given hour.
     *
     * @param hour Hour of this date. This is a floating point value. Fractions
     * are used for minutes and seconds.
    ...
    public JulianDate atModifiedJulianDate(double mjd) {
        Calendar clone = getCalendar();
        clone.setTime(new Date(Math.round((mjd - 40587.0) * 86400000.0)));
     ...
     *
     * @param unit {@link Unit} to truncate to
    ...
    public Calendar getCalendar() {
        return BaseBuilder.copyCalendar(cal);
    ...
    public double getTrueAnomaly() {
      //  int dayOfYear = cal.get(Calendar.DAY_OF_YEAR) - 1;
        ...
        double n1 = Math.floor(275 * m / 9);
        double n2 = Math.floor((m + 9) / 12);
        double n3 = (1 + Math.floor((y - 4 * floor(y / 4) + 2) / 3));
        int dayOfYear = (int) (n1 - (n2 * n3) + d - 30);
